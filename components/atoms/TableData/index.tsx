...
function createData(
  ...
  const rows = [
    ...
    createData('Layanan Live Streaming', 'Webinar Strategi Pencegahan Klaster Covid PTM Terbatas', '17/03/2023', '13.00', 'Ruang Sidang II Kantor Pusat UNS', 'Complete'),
  ]
                ...
                {rows.map((row) => (
                    <TableRow key={row.judulKegiatan} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                        ...
                        <TableCell align='center'>{
                            ...
                            : row.status === 'Complete' ? <Chip label={row.status} size='small' className='bg-complete text-white' /> : undefined
                        }</TableCell>
                    </TableRow>
                ))}
                </TableBody>
